...
const Update = ({updateIsOpen,setUpdateIsOpen,reviewId, getReviewData, movieId})=> {
  ...
  const findGetReviewData = ()=>{
    axios.get(process.env.REACT_APP_SERVER_URL + `/review/find/${cookies.userData.shortId}/${reviewId}`).then(res=>{
  ...
  const sendUpdateData = async()=>{
    return await axios.post(process.env.REACT_APP_SERVER_URL + "/review/update", updateData)
  }
  ...
  return (
    <div className="review-create-card">
      ...
      <div className="mb-3">
        ...
        <input type="text" className="form-control" name="name" id="name" defaultValue={updateData.author} disabled />
      </div>

      <div style={{textAlign:"right"}}>
        <button type="button" onClick={()=>onClickUpdateButton()} className="button grey-button-small" style={{marginRight:"5px"}}>UPDATE</button>
        <button type="button" onClick={()=>{setUpdateIsOpen(false)}} className="button grey-button-small">BACK</button>
      </div>      
    </div>
  )
}
