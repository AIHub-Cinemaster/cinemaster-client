...
import { setlike } from 'lib/api/like';
...
const ReviewCard = ({ review, getReviewData, accessType, setCreateAuth }) => {
  ...
  const onClickDeleteBtn = () => {
    if (window.confirm('삭제 하시겠습니까?')) {
      deleteReview(cookies.userData.shortId, review.movieId)
        .then((response) => {
  ...
  const onClickLike = () => {
    ...
    setlike(cookies.userData.shortId, review.reviewId).then((response) => {
      getReviewData(review.movieId);
    })
  };
  return (
      ...
      {updateIsOpen ? ( // 업데이트?
        ...
        <>
          <div className="review-card">
            ...
            <div className="review-content">
              <p className="white-small-font mb-4">{review.content}</p>
            </div>
            <div className="review-content-last">
                ...
                {accessType ? (
                  <>
                    {doYouLike ? (
                      <>
                        <span
                          className="material-symbols-outlined color-small-icons"
                          onClick={() => {
                            setDoYouLike(false);
                            onClickLike();
                          }}
                      ...
                      <>
                        <span
                          className="material-symbols-outlined grey-small-icons"
                          onClick={() => {
                            setDoYouLike(true);
                            onClickLike();
                          }}
              ...
              <div className="nav-right-wrap">
                ...
                <p className="grey-small-font set-inline">{review.author}</p>
