...
const MyPage = () => {
  ...
  useEffect(() => {
    getCartList()
      .then((res) => { //찜영화의 아이디만 담긴 배열
        res.data.result.map((item)=>{  //아이디 하나씩 넣어서
          getMovieInfoById(item).then(res=>{ // 정보를 가져온다.
  ...
  const getMovieInfoById = async (movie_id)=>{
    return await axios.get(`https://api.themoviedb.org/3/movie/${movie_id}?api_key=${API_KEY}&language=en-US`)
  }
